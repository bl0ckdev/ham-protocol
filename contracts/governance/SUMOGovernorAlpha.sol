...
// Modified to work in the SUMO system

// all votes work on underlying _sumoBalances[address], not balanceOf(address)
...
//
// Changes made by SUMO after audit:
//    Formatting, naming, & uint256 instead of uint
//    Since SUMO supply changes, updated quorum & proposal requirements
//    If any uint96, changed to uint256 to match SUMO as opposed to comp
...
contract GovernorAlpha {
    ...
    string public constant name = "SUMO Governor Alpha";

    /// @notice The number of votes in support of a proposal required in order for a quorum to be reached and for a vote to succeed
    function quorumVotes() public view returns (uint256) { return SafeMath.div(SafeMath.mul(sumo.initSupply(), 4), 100); } // 4% of SUMO

    /// @notice The number of votes required in order for a voter to become a proposer
    function proposalThreshold() public view returns (uint256) { return SafeMath.div(sumo.initSupply(), 100); } // 1% of SUMO
    ...
    constructor(address timelock_, address sumo_) public {
        timelock = TimelockInterface(timelock_);
        sumo = SUMOInterface(sumo_);
        guardian = msg.sender;
    }
    ...
    function propose(
        ...
        returns (uint256)
    {
        require(sumo.getPriorVotes(msg.sender, sub256(block.number, 1)) > proposalThreshold(), "GovernorAlpha::propose: proposer votes below proposal threshold");
        ...
        public
    {
        ...
        Proposal storage proposal = proposals[proposalId];
        require(msg.sender == guardian || sumo.getPriorVotes(proposal.proposer, sub256(block.number, 1)) < proposalThreshold(), "GovernorAlpha::cancel: proposer above threshold");
        ...
        internal
    {
        ...
        uint256 votes = sumo.getPriorVotes(voter, proposal.startBlock);
