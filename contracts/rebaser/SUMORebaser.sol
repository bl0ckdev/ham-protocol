...
import "../token/SUMOTokenInterface.sol";


contract SUMORebaser {
     ...
     * @notice Sets the reserve contract
     */
    event TreasuryIncreased(uint256 reservesAdded, uint256 sumosSold, uint256 sumosFromReserves, uint256 sumosToReserves);
    ...
    /// @notice pair for reserveToken <> SUMO
    ...
    /// @notice Whether or not this token is first in uniswap SUMO<>Reserve pair
    bool public isToken0;

    constructor(
        address sumoAddress_,
        ...
        public
    {
          ...
          (address token0, address token1) = sortTokens(sumoAddress_, reserveToken_);

          // used for interacting with uniswap
          if (token0 == sumoAddress_) {
              isToken0 = true;
          } else {
              isToken0 = false;
          }
          // uniswap SUMO<>Reserve pair
    ...
    */
        ...
        public
    {
        ...
        SUMOTokenInterface sumo = SUMOTokenInterface(sumoAddress);

        if (positive) {
            require(sumo.sumosScalingFactor().mul(uint256(10**18).add(indexDelta)).div(10**18) < sumo.maxScalingFactor(), "new scaling factor will be too big");
        }


        uint256 currSupply = sumo.totalSupply();
        ...
        // rebase
        uint256 supplyAfterRebase = sumo.rebase(epoch, indexDelta, positive);
        assert(sumo.sumosScalingFactor() <= sumo.maxScalingFactor());
        ...
        public
    {
        ...
        if (uniVars.amountFromReserves > 0) {
            ...
            sumo.transferFrom(reservesContract, uniswap_pair, uniVars.amountFromReserves);
            if (uniVars.amountFromReserves < uniVars.sumosToUni) {
                // if the amount from reserves > sumosToUni, we have fully paid for the yCRV tokens
                // thus this number would be 0 so no need to mint
                sumo.mint(uniswap_pair, uniVars.sumosToUni.sub(uniVars.amountFromReserves));
            }
        } else {
            // mint to uniswap
            sumo.mint(uniswap_pair, uniVars.sumosToUni);
        }

        // mint unsold to mintAmount
        if (uniVars.mintToReserves > 0) {
            sumo.mint(reservesContract, uniVars.mintToReserves);
        }

        // transfer reserve token to reserves
        if (isToken0) {
            SafeERC20.safeTransfer(IERC20(reserveToken), reservesContract, amount1);
            emit TreasuryIncreased(amount1, uniVars.sumosToUni, uniVars.amountFromReserves, uniVars.mintToReserves);
        } else {
            SafeERC20.safeTransfer(IERC20(reserveToken), reservesContract, amount0);
            emit TreasuryIncreased(amount0, uniVars.sumosToUni, uniVars.amountFromReserves, uniVars.mintToReserves);
        }
    }
        ...
        internal
    {
        ...
        // check if protocol has excess sumo in the reserve
        uint256 excess = sumo.balanceOf(reservesContract);


        uint256 tokens_to_max_slippage = uniswapMaxSlippage(token0Reserves, token1Reserves, offPegPerc);

        UniVars memory uniVars = UniVars({
          sumosToUni: tokens_to_max_slippage, // how many sumos uniswap needs
          amountFromReserves: excess, // how much of sumosToUni comes from reserves
          mintToReserves: 0 // how much sumos protocol mints to reserves
        });
        ...
        if (isToken0) {
            if (tokens_to_max_slippage > mintAmount.add(excess)) {
                ...
                uniVars.sumosToUni = mintAmount + excess;
                ...
                if (tokens_to_max_slippage > excess) {
                    ...
                    // swap up to slippage limit, taking entire sumo reserves, and minting part of total
