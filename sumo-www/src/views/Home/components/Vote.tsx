...
const Vote: React.FC<VoteProps> = () => {
  ...
  const handleVoteClick = useCallback(() => {
    delegate(sumo, account)
  }, [account, sumo])

  const fetchVotes = useCallback(async () => {
    const voteCount = await getVotes(sumo)
    const scalingFactor = await getScalingFactor(sumo)
    setTotalVotes(voteCount)
    setScalingFactor(scalingFactor)
  }, [sumo, setTotalVotes, setScalingFactor])
  ...
  const fetchDidDelegate = useCallback(async () => {
    const d = await didDelegate(sumo, account)
    if (d) {
      const amount = await getDelegatedBalance(sumo, account)
      setDelegatedBalance(amount)
    }
    setDelegated(d)
  }, [setDelegated, sumo, account, setDelegatedBalance])

  useEffect(() => {
    if (sumo && account) {
      fetchDidDelegate()
    }
  }, [fetchDidDelegate, sumo, account])
  ...
  return (
    <Card>
      <CardContent>
          ...
          <StyledCenter>
            ...
            <div style={{
              ...
              <div style={{
                  ...
                  marginTop: 4,
                  marginLeft: 4,
                }}>{`/ ${Number(new BigNumber(160000).multipliedBy(scalingFactor).toFixed(0)).toLocaleString()} SUMO`}</div>
            </div>
          </StyledCenter>
        </StyledResponsiveWrapper>
        <Spacer />
        ...
        <Spacer />
        {!delegated ? (
          <Button text="Delegate to save SUMO" onClick={handleVoteClick} />
        ) : (
          <div>
            <StyledDelegatedCount>Delegating: {Number(delegatedBalance.multipliedBy(scalingFactor).toFixed(0)).toLocaleString()} SUMO</StyledDelegatedCount>
        ...
        <div style={{
          ...
          <p>NOTE: You must harvest your SUMOs BEFORE 7am UTC Thursday 8/13 - very soon.</p>
          <p>Hold them in your wallet until 10PM UTC Sunday 8/16 for your delegation to save SUMO</p>
        </div>
          <div style={{
            display: 'flex',
            alignItems: 'center',
          ...
          <StyledLink target="__blank" href="https://twitter.com/SumoFinance/status/1293660938906869760">More Info</StyledLink>
        </div>
      </CardContent>
    </Card>
  )
